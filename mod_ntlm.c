 ...
 * Modified to work with reverse proxyies and compile on Mac OS X
 * by Michael Baltaks <mbaltaks - gmail - com> December 2006.
 * 
 ...
 * reproduced.
 *
 *  $Id: mod_ntlm.c 46 2008-03-31 20:34:51Z mbaltaks $
 *  
 */

#define VERSION "mod_ntlm2-0.3"
...
note_ntlm_auth_failure(request_rec * r)
{
    ...
    apr_table_setn(r->err_headers_out, (PROXYREQ_PROXY == r->proxyreq) ? "Proxy-Authenticate" : "WWW-Authenticate", line);
    if (crec->ntlm_basic_on) {
        line = apr_pstrcat(r->pool, "Basic realm=\"", crec->ntlm_basic_realm, "\"", NULL);
        apr_table_addn(r->err_headers_out, (PROXYREQ_PROXY == r->proxyreq) ? "Proxy-Authenticate" : "WWW-Authenticate", line);
    }
}
...
send_ntlm_challenge(request_rec * r, ntlm_config_rec * crec, int win9x)
{
    ...
    apr_table_setn(r->err_headers_out, (PROXYREQ_PROXY == r->proxyreq) ? "Proxy-Authenticate" : "WWW-Authenticate", apr_psprintf(r->pool, "%s %s", NTLM_AUTH_NAME, challenge));
    log(r, APLOG_INFO, "send %s \"%s %s\"",(PROXYREQ_PROXY == r->proxyreq) ? "Proxy-Authenticate" : "WWW-Authenticate", NTLM_AUTH_NAME, challenge);
...
{
    ntlm_config_rec *crec = (ntlm_config_rec *) ap_get_module_config(r->per_dir_config, &ntlm_module);
    const char *auth_line = apr_table_get(r->headers_in, (PROXYREQ_PROXY == r->proxyreq) ? "Proxy-Authorization" : "Authorization");
